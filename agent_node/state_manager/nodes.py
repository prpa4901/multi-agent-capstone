...
from agent_node.state_manager.agents import intent_agent, github_agent, clab_topology_agent, drift_manager_agent, llm
...
def clab_topology_agent_node(state: State) -> Command[Literal["state_supervisor"]]:
    ...
    return Command(
        update={
            "messages": [
                HumanMessage(content=result["messages"][-1].content, name="clab_topology_agent")
            ]
        },
        # Always report back to the state_supervisor when done
        goto="state_supervisor",
...
def drift_manager_agent_node(state: State) -> Command[Literal["state_supervisor"]]:
    ...
    return Command(
        update={
            "messages": [
                HumanMessage(content=result["messages"][-1].content, name="drift_manager_agent")
            ],
        },
...
state_supervisor_node = make_supervisor_node(llm, ["intent_agent", "github_agent", "clab_topology_agent", "drift_manager_agent"])
